...
omega_n_outer = sqrt(p.k_p_pos);  
zeta_outer = p.k_d_pos / (2 * omega_n_outer);  
settling_outer = 4 / (zeta_outer * omega_n_outer);  
...
% Calculated characteristics:
I_roll = p.I(1,1);
omega_n_inner = sqrt(p.k_p_att / I_roll);  
zeta_inner = p.k_d_att / (2 * sqrt(p.k_p_att * I_roll));  
settling_inner = 4 / (zeta_inner * omega_n_inner);  
...
figure('Name','Stable Time-Scale Separation (Optimized Gains)','Color','w','Position',[50 50 1800 1200]); % Increased window size
% Main 3D view
...
while t < tspan(2)
    ...
    [~, x_new] = ode45(@(t, x) quadEOM(t, x, u, p), [t, t+0.01], x, opts);
    x = x_new(end,:)';
...
legend('Actual','Desired'); title('Z Position (Complex, No Oscillation)');
ylim([0 1]);
...
%% ===== TRAJECTORY WITH SMOOTH TRANSITION (COMPLEX 3D) =====
function [pos_des, vel_des, acc_des, phase] = get_trajectory_with_takeoff(t, p)
    ...
    if t < takeoff_end
        ...
        ddh = (60*s - 180*s^2 + 120*s^3) / p.takeoff_duration^2;
    ...
    elseif t < hold_end
        ...
        % TRACKING complex 3D trajectory: figure-8 in XY + vertical sine wave
        t_track = t - hold_end;
        omega = 2*pi / p.traj_period;
        phase = 'TRACKING';
        
        x = p.traj_radius * sin(omega * t_track);
        y = (p.traj_radius/2) * sin(2 * omega * t_track);
        z = p.traj_height + 0.2 * sin(0.5 * omega * t_track);
        
        vx = p.traj_radius * omega * cos(omega * t_track);
        vy = (p.traj_radius/2) * 2 * omega * cos(2 * omega * t_track);
        vz = 0.2 * 0.5 * omega * cos(0.5 * omega * t_track);
        
        ax = -p.traj_radius * omega^2 * sin(omega * t_track);
        ay = -(p.traj_radius/2) * 4 * omega^2 * sin(2 * omega * t_track);
        az = -0.2 * 0.25 * omega^2 * sin(0.5 * omega * t_track);
