...
%% ===== TRAJECTORY WITH SMOOTH TRANSITION (FIGURE-8) =====
%% ===== TRAJECTORY WITH SMOOTH TRANSITION (SPIRAL) =====
function [pos_des, vel_des, acc_des, phase] = get_trajectory_with_takeoff(t, p)
    ...
    if t < takeoff_end
        % TAKEOFF with smooth polynomial (same as before)
        phase = 'TAKEOFF';
    ...
    elseif t < hold_end
        ...
        % TRACKING: SPIRAL upward trajectory
        t_spiral = t - hold_end;

        omega = 2*pi / p.traj_period;
        spiral_radius = p.traj_radius * (1 - t_spiral / (p.traj_period * 2)); % radius shrinks over time
        spiral_height_rate = 0.1; % upward speed in m/s

        phase = 'TRACKING';

        x = spiral_radius * cos(omega * t_spiral);
        y = spiral_radius * sin(omega * t_spiral);
        z = p.traj_height + spiral_height_rate * t_spiral;

        vx = -spiral_radius * omega * sin(omega * t_spiral) - (p.traj_radius / (p.traj_period * 2)) * cos(omega * t_spiral);
        vy = spiral_radius * omega * cos(omega * t_spiral) - (p.traj_radius / (p.traj_period * 2)) * sin(omega * t_spiral);
        vz = spiral_height_rate;

        ax = -spiral_radius * omega^2 * cos(omega * t_spiral) + 2 * (p.traj_radius / (p.traj_period * 2)) * omega * sin(omega * t_spiral);
        ay = -spiral_radius * omega^2 * sin(omega * t_spiral) - 2 * (p.traj_radius / (p.traj_period * 2)) * omega * cos(omega * t_spiral);
        az = 0;
